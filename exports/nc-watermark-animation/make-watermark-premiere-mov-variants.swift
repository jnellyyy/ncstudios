  ...
  for y in 0..<height {
    for x in 0..<width {
      ...
      if alpha == 0 {
        ...
        pixels[offset + 1] = UInt8((UInt16(variant.red) * UInt16(alpha)) / 255)
        pixels[offset + 2] = UInt8((UInt16(variant.green) * UInt16(alpha)) / 255)
        pixels[offset + 3] = UInt8((UInt16(variant.blue) * UInt16(alpha)) / 255)
      }
    }
  }
...
func writeVariant(_ variant: Variant) throws {
  let outputURL = exportURL.appendingPathComponent("nc-watermark-animation-\(variant.name)-transparent-premiere-upright.mov")
