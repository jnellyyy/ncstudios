NC Studio watermark reveal

Use in Premiere Pro:
- Best option: import the PNG sequence from frames/nc_watermark_0000.png.
- The sequence is transparent, 1920 x 1080, 30fps, 4 seconds.
- There is also a zipped copy: nc-watermark-animation-png-sequence.zip.
- Preview image: nc-watermark-animation-preview.png.
- APNG preview: nc-watermark-animation-transparent.apng.

Website loading screen:
- index.html and docs/index.html now use the same `[nc studio]` to `[nc]` reveal.

Video note:
- This machine does not have a working encoder that can turn the transparent sequence into MP4/MOV directly.
- For Premiere Pro, use the PNG sequence. It keeps transparency, unlike a normal MP4.
- Open intro-export.html to preview it, or use the ready zip in this folder.
